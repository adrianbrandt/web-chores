import { validateEmail, validatePassword, validatePhoneNumber, validate } from './validate';
...
describe('Validation Utils', () => {
  ...
  describe('validate', () => {
    it('should not throw error for valid validations', () => {
      ...
      expect(() => validate(validValidations)).not.toThrow();
    });
    ...
    it('should throw error for first invalid validation', () => {
      ...
      expect(() => validate(invalidValidations)).toThrow('First error');
      expect(Errors.Validation).toHaveBeenCalledWith({
        message: 'First error',
    ...
    it('should not throw error if invalid validation has no message', () => {
      ...
      expect(() => validate(validations)).not.toThrow();
    });
  });
});
